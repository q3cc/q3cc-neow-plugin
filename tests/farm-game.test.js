...
test('浇水和收获支持用 1-5 这种连续地块范围', async () => {
  const t = await env()
  try {
    const state = getFarmState('range-tending')
    state.seeds = {}
    buySeeds(state, 'radish', 5)
    plantSeed(state, '1-5', 'radish', 0)

    const watered = waterPlots(state, '1-5', 1)
    assert.equal(watered.ok, true)
    assert.deepEqual(watered.plots.map(plot => plot.plotId), [1, 2, 3, 4, 5])
    assert.equal(watered.staminaCost, 10)

    const readyAt = Math.max(...state.plots.slice(0, 5).map(plot => plot.readyAt))
    const harvested = harvestPlots(state, '1-5', readyAt + 1)
    assert.equal(harvested.ok, true)
    assert.deepEqual(harvested.harvested.map(item => item.plotId), [1, 2, 3, 4, 5])
    assert.equal(harvested.harvested.every(item => item.count === 1), true)
  } finally { await cleanup(t) }
})
