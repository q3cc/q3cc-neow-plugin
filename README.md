...
- `/farm plant <地块号|起始-结束> <作物别名>` - 在指定地块或连续地块播种
- `/farm water <地块号|起始-结束|all>` - 给一块地、连续地块或全部可浇水地块浇水
- `/farm harvest <地块号|起始-结束|all>` - 收获一块地、连续地块或全部成熟作物
...
- farm 现在每天会刷新 `3` 条每日任务，默认覆盖打开农场、买种子、播种、浇水、收获、卖作物与交订单这些常用动作
- `/farm` 总览页不再展开快捷操作，只保留一条 `/farm help` 引导提示；完整指令说明统一放在 `/farm help`
- `/farm` 总览会额外显示今日每日任务完成数，`/farm daily` 可查看进度、奖励与刷新倒计时
- `/farm plant` 支持连续地块范围，例如 `/farm plant 1-5 radish` 会尝试一次性给 `1-5` 号地同时播种
- `/farm water` 与 `/farm harvest` 也支持连续地块范围，例如 `/farm water 1-5`、`/farm harvest 1-5`
